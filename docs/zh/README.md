...
`bmlsub` 是面向 macOS / Apple Silicon 的本地 Headless 媒体生产核心。当前公开入口是 `bmlsub` CLI、`Pipeline` Python API 和 `CredentialService`。本页只描述 `Project/bmlsub` 当前代码已经实现的能力。
...
虽然 `ProductionOperation` 模型中存在 `remux` 枚举，但当前 CLI 不接受该 operation，`normalize_profile()` 也不允许执行；因此 1.0.0 不提供独立 remux 功能。
